...
def load_models(pepsi_model_path: str, coca_model_path: str) -> tuple:
    """Load models"""
    try:
        pepsi_model = YOLO(pepsi_model_path)
        coca_model = YOLO(coca_model_path)
        return pepsi_model, coca_model
...
def detect_cans(frame: cv2.Mat, models: tuple) -> tuple:
    ...
    pepsi_model, coca_model = models
    pepsi_results = pepsi_model.predict(source=frame, verbose=False)
    coca_results = coca_model.predict(source=frame, verbose=False)
    return pepsi_results, coca_results

def draw_bounding_box(frame: cv2.Mat, bounding_box: tuple, color: tuple) -> None:
...
def count_cans_crossing_yellow_line(frame: cv2.Mat, models: tuple) -> tuple:
    ...
    pepsi_model, coca_model = models
    pepsi_results, coca_results = detect_cans(frame, models)
    pepsi_counter = 0
    coca_counter = 0
    for result in pepsi_results:
        for bounding_box in result.boxes:
            if bounding_box.cls == PEPSI_CAN_CLASS:
                ...
                draw_bounding_box(frame, box, BLUE_COLOR)
                line_x = int(frame.shape[1] * YELLOW_LINE_HEIGHT_RATIO)
    ...
    for result in coca_results:
        for bounding_box in result.boxes:
            if bounding_box.cls == COCA_CAN_CLASS:
                box = [int(coord) for coord in bounding_box.xyxy.tolist()[0]]
                draw_bounding_box(frame, box, RED_COLOR)
                line_x = int(frame.shape[1] * YELLOW_LINE_HEIGHT_RATIO)
                if check_intersection(box, line_x):
                    coca_counter += 1
    return pepsi_counter, coca_counter

def display_frame(frame: cv2.Mat, pepsi_counter: int, coca_counter: int) -> None:
...
def end_counting_session(pepsi_counter: int, coca_counter: int):
...
def main():
    # Load models
    pepsi_model, coca_model = load_models("pepsi_best.pt", "coca_best.pt")
    if pepsi_model is None or coca_model is None:
        return
    ...
    try:
        while True:
            ...
            if key == ord("q"):
                ...
                end_counting_session(pepsi_counter, coca_counter)

            if cooldown_counter == 0:
                pepsi_count, coca_count = count_cans_crossing_yellow_line(frame, (pepsi_model, coca_model))
                pepsi_counter += pepsi_count
                coca_counter += coca_count
                if pepsi_count > 0 or coca_count > 0:
                    cooldown_counter = COOLDOWN

            if cooldown_counter > 0:
                cooldown_counter -= 1

            display_frame(frame, pepsi_counter, coca_counter)
